...
- **服务器实例**: `i-uf64o1e6a5vwu4boebya`
- **默认用户**: `root`
- **应用端口**: `5050`
- **Web URL**: `http://139.224.226.63:5050`
...
### 2.3 新服务器 (139.224.226.63) 专用部署
由于该服务器原先运行了 Nginx (Port 80)，需要使用专用脚本进行清理和部署。

**在本地创建部署脚本：**
1. 在本地项目根目录新建文件 `remote_deploy.sh`，内容见附件。
2. 或直接在服务器上执行以下命令：
...
# 执行一键部署 (会自动处理端口冲突)
curl -s https://raw.githubusercontent.com/wuyongming1984/pear-admin-flask/master/deploy.sh | bash
# 注意：如果 GitHub 访问不通，请手动复制 remote_deploy.sh 内容到服务器执行
```

**验证部署：**
```bash
docker-compose ps
# 访问 http://139.224.226.63:5050
```
